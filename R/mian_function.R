main_fun <- function(input_data = default_input_data) {
  ...
  if (ui_yeah("Do you want to proceed with column name?",
    ...
    if (0 < col_index & col_index <= ncol(input_data)){
      ...
      ui_warn("Column index should be between {ui_value(1)} and {ui_value(column_numbers)}")
  ...
  while (cond_stop == 0) {
    ...
    if (ui_yeah(message,
      ...
      if (selection == 1) {
        ...
        ui_info("Please provide a number as the window for rolling standard deviation:")
      ...
      if (selection == 6) {
        ...
        ui_info("Please provide the column index or name of the second input driver.")
        column_numbers <- ncol(input_data)
        ui_info("There are {ui_value(column_numbers)} columns in the input data.")
        column_names <- colnames(input_data)
        ...
        selection_object[[counter]] <- list(
          "Input entry" = input_entry_name,
          "Transformation" = paste0("Spread w.r.t ",
        ...
        ui_done("Transformation and tests are added to the meta data object!")
        input_data <- cbind(input_data, output_driver)
        colnames(input_data)[ncol(input_data)] <- paste0("Spread w.r.t ",
                                                         colnames(input_data)[col_index_2])
        ui_done("Transformed column is added to input data!")
      ...
      if (selection == 7) {
        ...
        selection_object[[counter]] <- list(
          "Input entry" = input_entry_name,
          "Transformation" = paste0("Ratio w.r.t ",
                                    colnames(input_data)[col_index_2]),
          "Normality at 0.05" =
            (normality_test$p.value > 0.05),
          "Stationarity at 0.05" =
        ...
        ui_done("Transformation and tests are added to the meta data object!")
        input_data <- cbind(input_data, output_driver)
        colnames(input_data)[ncol(input_data)] <- paste0("Ratio w.r.t ",
                                                         colnames(input_data)[col_index_2])
        ui_done("Transformed column is added to input data!")
      ...
      if (selection == 8) {
        ...
        selection_object[[counter]] <- list(
          "Input entry" = input_entry_name,
          "Transformation" = paste0("Product with ",
                                    colnames(input_data)[col_index_2]),
          "Normality at 0.05" =
            (normality_test$p.value > 0.05),
          "Stationarity at 0.05" =
        ...
        ui_done("Transformation and tests are added to the meta data object!")
        input_data <- cbind(input_data, output_driver)
        colnames(input_data)[ncol(input_data)] <- paste0("Product with ",
                                                         colnames(input_data)[col_index_2])
        ui_done("Transformed column is added to input data!")
